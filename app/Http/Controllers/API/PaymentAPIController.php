...
class PaymentAPIController extends AppBaseController
{
    ...
    public function pay(Request $request, $hostId)
    {
        $host = HostDetail::findOrFail($hostId);

        $amount = $request->amount;
        $phone = $host->phone_number;

        $response = $this->paymentService->initiateSTKPush($amount, $phone);
    ...
    public function callback(Request $request)
    {
        ...
        if (isset($data['Body']['stkCallback'])) {
            ...
            $payment = Payment::where('checkout_request_id', $callback['CheckoutRequestID'])->first();
